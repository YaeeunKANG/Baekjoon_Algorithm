...
for x in range(0, len(card)-2):
  for y in range(1, len(card)-1):
    for z in range(2, len(card)):
      if (card[x] == card[y] or card[x] == card[z] or card[y] == card[z]):
        continue
      elif (card[x] + card[y] + card[z] > sum) and (card[x] + card[y] + card[z] <= M) :
